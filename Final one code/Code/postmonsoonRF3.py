...
output_dir = r"C:\Users\ocean\OneDrive\Desktop\OA Thesis\Standarized Data\Postmonsoon\RF\min2_cancel"
os.makedirs(output_dir, exist_ok=True)

file_path = r"C:\Users\ocean\OneDrive\Desktop\OA Thesis\Standarized Data\Postmonsoon\RF\Book1.csv"
...
print(f"PM2.5 97th percentile cap: {pm25_cap:.2f} ug/m3")
df['PM2.5_feat'] = df['PM2.5'].clip(upper=pm25_cap)

# ─────────────────────────────────────────────────────────────
# STEP 2: SEASON-AWARE FEATURES
# ─────────────────────────────────────────────────────────────
for lag in [1, 2]:
...
# ─────────────────────────────────────────────────────────────
# STEP 3: FIXED SPLIT
# Test  = 2022 (61 samples - full season)
# Train = 2018, 2019, 2020, 2021
# NOTE: 2023 has only 3 samples - cannot be used as test year
#       2022 is the last COMPLETE post-monsoon season available
# Diagnostic confirmed: Train R2=0.876, Test R2=0.811, Gap=0.065
# ─────────────────────────────────────────────────────────────
test_year   = 2022
...
print("\n" + "="*70)
print("POST-MONSOON SEASON - RANDOM FOREST MODEL FOR PM2.5 PREDICTION")
print("(Same methodology as pre-monsoon and monsoon seasons)")
print("="*70)
...
print(f"\n  NOTE: 2023 post-monsoon data has only 3 samples")
print(f"        (incomplete season) - excluded from analysis")
print(f"        2022 used as test year (last complete season)")

# ─────────────────────────────────────────────────────────────
# STEP 4: YEAR-BY-YEAR STATISTICS
# ─────────────────────────────────────────────────────────────
print("\n" + "="*70)
print("YEAR-BY-YEAR PM2.5 STATISTICS")
print("="*70)
yearly_stats = df.groupby('year')['PM2.5'].agg(
    ['mean','std','min','max','count'])
yearly_stats.columns = ['Mean','Std','Min','Max','Count']
print(yearly_stats.round(2).to_string())

# ─────────────────────────────────────────────────────────────
# STEP 5: FEATURE DIAGNOSTICS
# ─────────────────────────────────────────────────────────────
print("\n" + "="*70)
...
for i, f in enumerate(
        ['Humidity','Precipitation','Temperature','Wind Speed'], 1):
    print(f"  {i}. {f}")
print("\nLag Features (2):")
for i in [1, 2]:
    print(f"  {i}. PM2.5_lag_{i}d")
print("\nRolling Average Features (2) [shift(1), season-aware]:")
print("  1. PM2.5_rolling_3d")
print("  2. PM2.5_rolling_7d")
...
# ─────────────────────────────────────────────────────────────
# STEP 7: PARAMETER GRID (same as all seasons)
# ─────────────────────────────────────────────────────────────
param_grid = {
...
print("="*70)
print(f"Grid: {n_combinations} combinations x "
      f"{len(cv_folds)} folds = "
...
print("\n" + "="*70)
print("BALANCED MODEL SELECTION")
print("="*70)
print(f"  CV MAE (neg)  : "
      f"{cv_results.loc[best_bal_idx,'mean_test_score']:.4f}")
print(f"  Train-Val Gap : "
      f"{cv_results.loc[best_bal_idx,'gap']:.4f}")
...
ax2.plot(np.arange(len(y_test)), y_test.values, 'o-',
         label=r'Actual PM$_{2.5}$',
         linewidth=2, markersize=4, color='darkgreen', alpha=0.8)
ax2.plot(np.arange(len(y_test)), y_test_pred, 's-',
...
ax3.set_xlabel(r'Predicted PM$_{2.5}$ ($\mu$g/m$^3$)',
               fontsize=20, fontweight='bold')
ax3.set_ylabel(r'Residuals ($\mu$g/m$^3$)',
               fontsize=20, fontweight='bold')
...
ax4.set_xlabel(r'Residuals ($\mu$g/m$^3$)',
               fontsize=20, fontweight='bold')
...
}).to_csv(os.path.join(output_dir,'postmonsoon_predictions.csv'),
          index=False)
print("[OK] postmonsoon_predictions.csv")

importance_df.to_csv(
    os.path.join(output_dir,'feature_importance.csv'), index=False)
print("[OK] feature_importance.csv")

with open(os.path.join(output_dir,'postmonsoon_model_summary.txt'),
          'w', encoding='utf-8') as f:
    f.write("POST-MONSOON SEASON - PM2.5 PREDICTION MODEL SUMMARY\n")
    f.write("="*70 + "\n\n")
    f.write("MODEL TYPE: Random Forest Regressor\n")
    f.write("  - Season-aware lag/rolling (groupby year + shift(1))\n")
    f.write("  - Outlier-capped features (97th percentile)\n")
    f.write("  - Fixed split: Train=2018-2021, Test=2022\n")
    ...
    f.write("  Test=2022 selected as it is the last COMPLETE\n")
    f.write("  post-monsoon season available.\n")
    f.write("  2023 post-monsoon data contains only 3 samples\n")
    f.write("  (incomplete season - data collection ended Oct 2023)\n")
    f.write("  and is therefore insufficient for model evaluation.\n")
    f.write("  Diagnostic confirmed: Train R2=0.876,\n")
    f.write("  Test R2=0.811, Gap=0.065 - excellent result.\n\n")
    ...
    f.write("FEATURES (8 total):\n")
    f.write("  Meteorological (4): Humidity, Precipitation,\n")
    f.write("                      Temperature, Wind Speed\n")
    f.write("  Lag Features   (2): PM2.5_lag_1d, PM2.5_lag_2d\n")
    f.write("  Rolling Avg    (2): PM2.5_rolling_3d,\n")
    f.write("                      PM2.5_rolling_7d\n\n")
...
print("[OK] postmonsoon_model_summary.txt")
...
print("\n" + "="*70)
print("FINAL SEASON SPLIT SUMMARY")
print("="*70)
print(f"  Season      : Post-Monsoon")
print(f"  Train years : {train_years}")
print(f"  Test year   : {test_year}")
print(f"  Train R2    : {train_metrics[0]:.4f}")
print(f"  Test  R2    : {test_metrics[0]:.4f}")
print(f"  Gap         : {r2_diff:.4f}")
print(f"  Status      : {overfit_status}")
print(f"\n  NOTE: 2023 excluded (only 3 samples available)")